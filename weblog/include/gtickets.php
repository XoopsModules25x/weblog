...
$gticket_default_timeout =  ( ! empty($xoopsModuleConfig['GTicket_timeout']) ) ? $xoopsModuleConfig['GTicket_timeout'] : 10 ;
define('GTICKET_DEFAULT_TIMEOUT', $gticket_default_timeout*60 );
...
class XoopsGTicket {
	...
	function getTicketHtml( $salt = '' , $timeout = GTICKET_DEFAULT_TIMEOUT )
	{
		return '<input type="hidden" name="XOOPS_G_TICKET" value="'.$this->issue( $salt , $timeout ).'" />' ;
	}

	// returns an object of XoopsFormHidden including theh ticket
	function getTicketXoopsForm( $salt = '' , $timeout = GTICKET_DEFAULT_TIMEOUT )
	{
		return new XoopsFormHidden( 'XOOPS_G_TICKET' , $this->issue( $salt , $timeout ) ) ;
	}

	// returns an array for xoops_confirm() ;
	function getTicketArray( $salt = '' , $timeout = GTICKET_DEFAULT_TIMEOUT )
	{
		return array( 'XOOPS_G_TICKET' => $this->issue( $salt , $timeout ) ) ;
	}

	// return GET parameter string.
	function getTicketParamString( $salt = '' , $noamp = false , $timeout=GTICKET_DEFAULT_TIMEOUT )
	{
	    return ( $noamp ? '' : '&amp;' ) . 'XOOPS_G_TICKET=' . $this->issue( $salt, $timeout ) ;
	}

	// issue a ticket
	function issue( $salt = '' , $timeout = GTICKET_DEFAULT_TIMEOUT )
	{
		// create a token
		list( $usec , $sec ) = explode( " " , microtime() ) ;
		$token = crypt( $salt . $usec . $sec ) ;
